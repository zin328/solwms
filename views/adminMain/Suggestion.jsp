...
<head>
    ...
    <style>
        ...
        .sugg-table th:nth-child(1),
		.sugg-table td:nth-child(1) {
    		width: 15%;
		}

		.sugg-table th:nth-child(2),
		.sugg-table td:nth-child(2) {
    		width: 65%;
		}	

		.sugg-table th:nth-child(3),
		.sugg-table td:nth-child(3) {
    		width: 20%;
		}
...
<body style="font-family: Arial, sans-serif; background-color: #f4f4f4;">
    ...
    <div class="sugg-container">
           <h2 class="sugg-header">건의사항</h2>
            
            <!-- 검색창 추가 -->
            <div class="search-container">
    			<form class="search-bar" method="get" action="/suggestion" style="display: inline-block; margin-left: 20px;">
        			<input type="text" id="search-title" name="search" placeholder="검색" value="${search}" />
        			<button type="submit" class="search-btn">검색</button>
        			
    			</form>
    			<button id ="addSuggBtn" class="add-sugg-btn">+</button>
    		</div>
        ...
        <!--건의사항 리스트 -->
           ...
           <tr>
           <th>${suggestion.sugNumber}</th>
           <td>
    		<a href="/adminMain/suggContent?sugNumber=${suggestion.sugNumber}" class="suggestion-title">
        		${suggestion.sugTitle}
    		</a>
			</td>
...
<div class="pagination">
    ...
    <c:if test="${not empty search}">
        <c:set var="queryParams" value="&search=${search}"/>
    </c:if>

    <c:if test="${currentPage > 1}">
        <a href="/suggestion?page=${currentPage - 1}${queryParams}&p=${currentPage - 1}">« 이전</a>
    </c:if>
    ...
    <c:forEach begin="1" end="${totalPages}" var="pageNum">
        <c:choose>
            ...
            <c:otherwise>
                <a href="/suggestion?page=${pageNum}${queryParams}">${pageNum}</a>
            </c:otherwise>
        </c:choose>
    </c:forEach>

    <c:if test="${currentPage < totalPages}">
        <a href="/suggestion?page=${currentPage + 1}${queryParams}&p=${currentPage + 1}">다음 »</a>
    </c:if>
</div>
</div>
...
<!-- 모달 창 -->
<div class="modal" id="sugg-modal">
<form method="post" action="/adminMain/addSuggestion" onsubmit="return suggestionCheck()" >
    <h2>건의사항 추가</h2>
    <input type="text" id="sugg-title" name="suggestion-title" placeholder="제목">
    <textarea id="sugg-content" name="suggestion-content" placeholder="내용"></textarea>
    
    <div class="button-container">
    	<button type="submit" id="saveSuggestionsBtn" class="save-btn">저장</button>
    	<button type="button" class="cancel-btn" onclick="closeSuggestionForm()">취소</button>
    </div>
   </form>
</div>
...
document.addEventListener("DOMContentLoaded", function() {
    // ✅ `+` 버튼 클릭 시 모달 열기
    document.getElementById("addSuggBtn").addEventListener("click", function() {
        openSuggestionForm();
    });

    // ✅ 폼 제출 시 유효성 검사 실행
    document.getElementById("sugg-form").addEventListener("submit", function(event) {
        if (!suggCheck(event)) {
...
function openSuggestionForm() {
    document.getElementById("sugg-modal").style.display = "block";
    document.getElementById("overlay").style.display = "block";
}

// ✅ 모달 닫기
function closeSuggestionForm() {
    document.getElementById("sugg-modal").style.display = "none";
    document.getElementById("overlay").style.display = "none";
}

// ✅ 유효성 검사 함수 수정
function noticeCheck(event) {
    var noticeTitle = document.getElementById("sugg-title").value.trim();
    var noticeContent = document.getElementById("sugg-content").value.trim();
