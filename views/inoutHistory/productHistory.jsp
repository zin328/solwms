...
<div class="main-content">
    ...
    <form method="get" action="/searchProductHistory" class="search-form">
        ...
        <input type="text" id="searchKeyword" name="searchKeyword" value="${item}">

        <label for="wareName">물류센터:</label>
        <input type="text" id="wareName" name="wareName" value="${model}">
    ...
    <div id="page" class="pagination-container">
        <div class="pagination">
            <c:if test="${begin > pageNum }">
                <a href="productHistory?p=${begin-1 }" class="pagination-link">« 이전</a>
            </c:if>
            <c:forEach begin="${begin }" end="${end}" var="i">
                <a href="productHistory?p=${i}" class="pagination-link ${i == pa ? 'active' : ''}">${i}</a>
            </c:forEach>
            <c:if test="${end < totalPages }">
                <a href="productHistory?p=${end+1}" class="pagination-link">다음 »</a>
            </c:if>
        </div>
    </div>
</div>
